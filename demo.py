...
def page1(is_demo=False):
    ...
    ans_list = ["Alzheimer's disease(AD)", "cognitive normal(CN)", "mild cognitive impairment(MCI)", "early mild cognitive impairment(EMCI)",
    ...
    while 1:
        try:
            ...
            emci_content = [pywebio.output.put_markdown("Early mild cognitive impairment")]
            mci_content = [pywebio.output.put_markdown("Mild cognitive impairment")]
            lmci_content = [pywebio.output.put_markdown("Late mild cognitive impairment")]
            ...
            pywebio.output.put_row(
                [pywebio.output.put_collapse("CN", cn_content, open=compare_ans("Cognitive normal(CN)", ans)),
                 pywebio.output.put_collapse("EMCI", emci_content, open=compare_ans("Early mild cognitive impairment(EMCI)", ans)),
                 pywebio.output.put_collapse("MCI", mci_content, open=compare_ans("Mild cognitive impairment(MCI)", ans)),
                 pywebio.output.put_collapse("LMCI", lmci_content, open=compare_ans("Late mild cognitive impairment(LMCI)", ans)),
            ...
            if is_demo is False:
                try:
                    inpic = pywebio.input.file_upload(label="Upload medical images(.nii)")
                    ...
                    pywebio.output.toast("Input error, please upload medical imaging files(.nii)", color="warn")
                    refresh()
            ...
            try:
                ...
                pywebio.output.toast("Input processing error, please upload medical imaging files(.nii)\tshould be greater than：(168x168x168)", color="warn")
                refresh()
